...
export default function SearchPage ({ props }) {
  ...
  const programme_type = searchParams.get('programme_type');
  const school_slug = searchParams.get('school_slug');
  ...
  const fetchData = async (Params = { discipine: '', degree_type: [], attendance: [], format: [], duration: [], tuition: '', countries: [] }) => {
    ...
    if (countryName == 'Where?' || countryName == 'where?'  || countryName == 'Anywhere' || countryName === null) {
      url = `${baseUrl}?name=${courseName.replace(' ', '%20')}&page=${currentPage + 1}&ordering=${'random'}&`;
    } else {
      url = `${baseUrl}?name=${courseName.replace(' ', '%20')}&country=${countryName.replace(' ', '-').toLowerCase()}&page=${currentPage + 1}&&ordering=${'random'}&`;
    }
    if (discipline) {
      url = url + `discipline=${discipline}&`;
    }
    if(school_slug){
      url = url + `school_slug=${school_slug}&`;
    }
      ...
      <div className='lg:flex flex-row mt-24 justify-start w-screen max-w-full mb-10 mx-auto'>
        <aside className='hidden lg:block px-8 shadow-2 py-4 lg:w-100 h-fit sticky left-0 top-24  bg-white max-w-full'>
          ...
          <div className=' h-fit '>
            ...
            <div className='block'>
              <div className='text-lg  py-2 flex gap-x-28 border-t border-light_black border-opacity-20 justify-between' onClick={() => { setMastersInfo(!showMasters); }}><div>Graduates</div>  <button className=''>
            ...
            <div id='shortcourse' className='block'>
              ...
              {/* <div className='text-lg py-2 flex gap-x-28 border-t border-light_black border-opacity-20 justify-between' onClick={() => { setDocInfo(!showDoc); }}><div></div>  <button className=''>
                    ...
                    !showFilterBar
                      ? <aside className='lg:block px-8 shadow-2 py-4 lg:w-100 h-fit sticky left-0 top-24  bg-white max-w-full'>
                        ...
                        <div className=' h-fit '>
                          ...
                          <div className='block'>
                            <div className='text-lg  py-2 flex gap-x-28 border-t border-light_black border-opacity-20 justify-between' onClick={() => { setMastersInfo(!showMasters); }}><div>Postgraduates</div>  <button className=''>
                              {showMasters ? <FiChevronUp /> : <FiChevronDown />}
                                                                                                                                                                                                             </button>
                            </div>
                            <div className={showMasters ? 'block py-4' : 'hidden'}>
        ...
        <div className=' flex w-full'>
          <div className='mb-4 w-full px-4'>
            <h1 className='text-3xl w-fit'>{`"${courseName ? courseName.replace('-', ' ').split(' ').map(word => word.charAt(0).toUpperCase() + word.slice(1)).join(' ') : ''}"`} Courses</h1>
              ...
              <div className='flex gap-x-2 justify-between md:gap-x-4 items-center text-light_black w-full'>
                <div className='flex justify-between gap-x-4 my-4 items-center w-full  lg:w-3/12'>
                  ...
                  <div>
                    <span>Order: </span>
                    <select
                        className='focus:outline-none p-2 rounded-md bg-white border  border-light_black border-opacity-30  w-20 md:w-32' vvalue={sortOrder}
